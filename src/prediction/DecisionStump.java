...
public class DecisionStump extends Node implements IDecisionStump {
    ...
    private void categoricalSplit(){        
        ...
        for (ArrayList<Double> br : branches) {
            double temp = br.get(0)/dataPartition.studentIndexes.size();
            varianceReduction += temp * br.get(2); //branch variance weighted by percentage of students in branch
        }
    ...
    public String toString(){
        int[][] rightPredictions = rightPredictions();
        ...
        s+= "]\n";
        s += "Initial variance: "+ initialVariance + "; " + "Variance reduction: " + varianceReduction + "; " + "Final variance: " + finalVariance + "; Information Gain: " + entropy[entropy.length-1] + "\n";
        for(int i = 0; i <= thresholds.size(); i++){
            if(i==0)
    ...
    public double[] getEntropy(int[][] rightPredictions){
        double[] entropy = new double[branchProperties.size()+1];
        ...
        entropy[entropy.length-1] = informationGain;
        return entropy;
    }
    ...
    public static void main(String[] args) { //!TSO-010 12/10, PLO-132 18/16, BKO-801 21/19
        ...
        DecisionStump ds = new DecisionStump(data, 31, 13);
